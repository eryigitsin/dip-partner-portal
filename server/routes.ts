...
export function registerRoutes(app: Express): Server {
  ...
  app.get("/auth/linkedin", (req, res) => {
    const state = req.query.state as string;
    const authUrl = generateAuthUrl('linkedin', state);
    res.redirect(authUrl);
  });
  ...
  app.get("/auth/google/callback", async (req, res) => {
    try {
      ...
      if (!code) {
        return res.redirect('/auth?error=oauth_canceled');
      }
      ...
      // Fetch user info
      const oauthUser = await fetchUserInfo('google', tokens.access_token);
      ...
      if (user) {
        // Create JWT for Supabase
        const supabaseJWT = createSupabaseJWT(oauthUser);
        
        // Login user in our session
        req.login(user, (err) => {
          if (err) {
            console.error('Login error:', err);
          ...
          // Redirect with JWT for Supabase auth
          const redirectUrl = state ? `/auth?jwt=${supabaseJWT}&state=${state}` : `/auth?jwt=${supabaseJWT}`;
          res.redirect(redirectUrl);
        });
      } else {
        res.redirect('/auth?error=user_not_found');
      }
    } catch (error) {
  ...
  app.get("/auth/linkedin/callback", async (req, res) => {
    try {
      ...
      // Fetch user info
      const oauthUser = await fetchUserInfo('linkedin', tokens.access_token);
